...
function loadbayer()
  path = "../Ex 1/data-julia/bayerdata.jld"
  data = load(path, "bayerimg")
  data = data./255
  return data::Array{Float64,2} #480x320 Array{Float64,2}:  
end
...
function makeimage(r::Array{Float64,2},g::Array{Float64,2},b::Array{Float64,2})
  image = cat(3,r,g,b) # Stack color channels along third dimension
...
function debayer(r::Array{Float64,2},g::Array{Float64,2},b::Array{Float64,2})
  rbfilter = [.25 .5 .25; .5 1 .5; .25 .5 .25] # Bilinear Interpolation filter for Red and Blue values
  gfilter  = [0 .25 0; .25 1 .25; 0 .25 0]  # For Green values
  r = imfilter(r, rbfilter, "replicate")
  g = imfilter(g, gfilter, "replicate")
  b = imfilter(b, rbfilter, "replicate")
  return cat(3,r,g,b)::Array{Float64,3}
end
